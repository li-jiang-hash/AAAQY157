;window.onload = () => {
  ...
  let goodsList = [
      ...
      {goodsId: 4, goodsName: "OPPO Reno 2 ", goodsPrice: 2999.0},
      {goodsId: 5, goodsName: "HUAWEIMate40 pro", goodsPrice: 6999.0}
    ]
    //让商品加载到商品列表中
  ;(function onLoadInit() {
  ...
  this.AllGoods = (goodsNum, symbol) => {
    ...
    console.log("goodsNum----" + goodsNum)
    console.log("goodsAllNum----" + $('#goodsAllNum').innerHTML)
  }
  ...
  this.del = obj => {
    ...
    getTotal()
    //调用件数计算方法总件数减一
    AllGoods(obj.parentNode.parentNode.cells[3].childNodes[3].innerHTML, '-')
    ...
    console.log("cartArr-del" + cartArr.length)
  ...
  this.checkAll = () => {
    ...
    for (let i in goodsArr){
      goodsArr[i].checked = checkAll.checked
    }
    getTotal()
  }
  //选择/取消选则                                                                                      ++++679/
  this.selectOne = a => {
    //选中件数计算
    if (a.checked){
      $("#selectGoodsNum").innerHTML = parseInt($("#selectGoodsNum").innerHTML) + parseInt(a.parentNode.parentNode.cells[3].childNodes[3].innerHTML)
      console.log("goodsArr[i]选中的"+$("#selectGoodsNum").innerHTML)
    }else{
      $("#selectGoodsNum").innerHTML = parseInt($("#selectGoodsNum").innerHTML) - parseInt(a.parentNode.parentNode.cells[3].childNodes[3].innerHTML)

      console.log("goodsArr[i]没选中的"+$("#selectGoodsNum").innerHTML)
    }
  ...
  this.delSelect = () => {
    ...
    for (let i = 0; i < goodsArr.length; i++) {
      if (goodsArr[i].checked) {
        ...
        $("#goodsAllNum").innerHTML = Number($("#goodsAllNum").innerHTML) - Number(goodsArr[i].parentNode.parentNode.cells[3].childNodes[3].innerHTML)
        ...
        //   调用件数计算
        //  AllGoods(goodsArr[i].parentNode.parentNode.cells[3].childNodes[3].innerHTML, '-')
        console.log(goodsArr[i].parentNode.parentNode.cells[3].childNodes[3].innerHTML)
      }
    }
    // }
    //调用价格运算方法
